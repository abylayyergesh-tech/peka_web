...
export default function ShiftsPage() {
  ...
  const doOpen = useMutation({
    ...
    onSuccess: (s) => {
      message.success(`Смена №${s.number ?? s.id} открыта`);
      setOpenModal(false);
  ...
  const doClose = useMutation({
    ...
    onSuccess: (report) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["shifts"] });
      navigate(`/shifts/${report.shift.id}`);
    },
    onError: (e) => message.error(errorMessage(e)),
  });

  const warehouseName = (id: number) =>
    warehouses.data?.items.find((w) => w.id === id)?.name ?? `#${id}`;
  ...
  const columns: ColumnsType<ShiftOut> = [
    {
      ...
      dataIndex: "number",
      width: 80,
      render: (v: number | null, row) => <Link to={`/shifts/${row.id}`}>{v ?? row.id}</Link>,
    },
    ...
    },
    {
      title: "",
      width: 100,
      render: (_, row) => <Link to={`/shifts/${row.id}`}>Отчёт</Link>,
    },
  ];
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16, justifyContent: "space-between", width: "100%" }}>
        <h2 style={{ margin: 0 }}>Смены (касса)</h2>
        {canOperate && (
      ...
      >
        ...
        <Space direction="vertical" style={{ width: "100%" }} size={12}>
          {openShifts.data?.items.map((s) => (
            <Card key={s.id} type="inner" title={`Смена №${s.number ?? s.id}`}
              ...
              <Space style={{ marginTop: 8 }}>
                <Button onClick={() => navigate(`/shifts/${s.id}`)}>Отчёт по смене</Button>
                {canOperate && (
                  <Popconfirm
                    title="Закрыть смену?"
