...
export const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // Перемещение отдельным пунктом, хотя это тот же документ: в общей форме
      // документа строки набираются вслепую, а здесь лист строится по остаткам
      // склада-источника — видно, что и сколько можно увезти. Тот же экран есть
      // в кабинете сотрудника (peka_staff).
      { path: "/transfer", label: "Перемещение по складам", cap: "inventory.manage" },
  ...
  },
  {
    ...
    items: [
      ...
      // Развозка: рейсы курьеров поверх заказов на доставку. Живёт в продажах,
      // потому что планируют её по заказам; склада и денег не касается — отметка
      // «доставлено» ничего не списывает, выдачу проводит касса.
