...
|---|---|---|
| Сотрудники → карточка человека (клик по строке) | договоры, удостоверения, заявления | `staff.manage` |
...
Строка в списке сотрудников нажимается целиком и открывает карточку: данные,
**заявления** (тип, статус, о чём, когда подано — с провалом в само заявление),
личное дело и действия «Изменить» / «Уволить». Три ссылки в последней колонке
таблицы убраны: при девяти колонках их искали прокруткой.

Заявления показываются только при праве `request.approve` — их отдаёт
`GET /requests?employee_id=N`, и без права ручка ответила бы 403.

Решать заявление (одобрить/отклонить) из карточки нельзя намеренно: это работа
экрана «Все заявления», где видно очередь и права на стадию.

## Табель (`/payroll/timesheet`)

Сетка месяц × сотрудники, в клетке смены или отсутствие. **Селектор отдела** —
главный переключатель экрана: табель ведут по цехам, «Все отделы» остаётся для
расчётчиков. Клик по клетке открывает меню отметок (смены, Б, О, БС, В) —
отсутствия там же, где смены, а не на отдельном экране.

Кнопки: «Собрать из отметок» (явки + отпуска из заявлений), «Подтянуть отпуска»
(только отсутствия, без пересборки смен), «Выгрузить табель» (один лист по
выбранному отделу), «Книга за период» (полная книга с ведомостями), «Печать»
(A4 landscape, со строкой подписи начальника цеха).

Страница «Отметки смен» (`/attendance`) — соседний уровень: отдельные явки со
временем и геопозицией. Раньше её заголовок тоже назывался «Табель», и два разных
экрана было не отличить; теперь они называются по-разному и ссылаются друг на друга.
